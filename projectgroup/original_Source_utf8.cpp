...
void drawSky()
{
    ...
    // Clouds ΓÇö 5 clouds drifting across the sky
    float baseZ = -50.0f;
...
void drawEarth()
{
    // 1. Lush Grass Ground (Restored & Offset to prevent Z-fighting)
    glColor3f(0.25f, 0.45f, 0.15f);
    glBegin(GL_QUADS);
    glNormal3f(0, 1, 0);
    glVertex3f(-100.f, -0.05f, 100.f); glVertex3f(100.f, -0.05f, 100.f);
    glVertex3f(100.f, -0.05f, -100.f); glVertex3f(-100.f, -0.05f, -100.f);
    glEnd();

    // 1.5 Asphalt Road (Refined lane & Offset to prevent Z-fighting)
    glColor3f(0.22f, 0.22f, 0.25f); 
    glBegin(GL_QUADS);
    glNormal3f(0, 1, 0);
    glVertex3f(-100.f, -0.02f, 12.0f); glVertex3f(100.f, -0.02f, 12.0f);
    glVertex3f(100.f, -0.02f, 17.0f); glVertex3f(-100.f, -0.02f, 17.0f);
    glEnd();

    // Road Markings (Dashed white line & Offset to prevent Z-fighting)
    glColor3f(0.95f, 0.95f, 0.95f);
    for (float x = -98.0f; x <= 98.0f; x += 6.0f) {
        glBegin(GL_QUADS);
        glVertex3f(x, -0.015f, 14.4f); glVertex3f(x + 3.0f, -0.015f, 14.4f);
        glVertex3f(x + 3.0f, -0.015f, 14.6f); glVertex3f(x, -0.015f, 14.6f);
        glEnd();
    }
    ...
    // Dense Long Grass inside the rectangular hedge (Deterministic placement)
    for (float x = -5.5f; x <= 3.7f; x += 0.5f) {
        for (float z = 4.0f; z <= 7.0f; z += 0.5f) {
            // Static, deterministic jitter based on coordinates to eliminate shaking
            int seed = (int)((x + 50.0f) * 20.0f) * 31 + (int)((z + 50.0f) * 20.0f);
            float jx = (float)(seed % 10) / 20.0f - 0.25f;
            float jz = (float)((seed * 7) % 10) / 20.0f - 0.25f;
    ...
    // Rich, Dense Background Forest (Filling behind the buildings)
    // We cover a large organic grid from X = -95.0f to 95.0f, and Z = -6.0f down to -45.0f
    for (float z = -6.0f; z >= -45.0f; z -= 4.0f) {
        for (float x = -95.0f; x <= 95.0f; x += 4.0f) {
            // Check if we would collide with buildings (between Z = -6.0f and Z = -4.5f)
            if (z > -4.5f) {
                // Center building: X from -5.5f to 5.5f
                if (x >= -5.5f && x <= 5.5f) continue;
                // Left building: X from -20.5f to -9.5f
                if (x >= -20.5f && x <= -9.5f) continue;
            }

            // Check if we would collide with the three walkways (which extend down to Z = -15.0f)
            if (z >= -16.0f) {
                // Middle-left path: X from -9.5f to -5.5f
                if (x >= -9.5f && x <= -5.5f) continue;
                // Right path: X from 7.25f to 11.25f
                if (x >= 7.25f && x <= 11.25f) continue;
                // Far-left path: X from -26.25f to -22.25f
                if (x >= -26.25f && x <= -22.25f) continue;
            }

            // Generate deterministic pseudo-random seed based on grid coordinates
            int ix = (int)((x + 100.0f) * 10.0f);
            int iz = (int)((z + 100.0f) * 10.0f);
            int seed = ix * 1973 + iz * 9277 + 42;

            // Apply slight jitter for a natural, organic look (prevents mechanical grid look)
            float jx = (float)((seed % 7) - 3) * 0.35f;
            float jz = (float)(((seed / 7) % 7) - 3) * 0.35f;

            // Vary the scale of the trees to look like a real three-dimensional forest
            float scale = 0.85f + (float)(seed % 5) * 0.08f;

            // Select a beautiful mix of tree types: Oak, Willow, Pine, Jacaranda, Acacia, Evergreen
            int types[] = { 2, 3, 4, 5, 6, 0 }; 
            int treeType = types[seed % 6];

            glPushMatrix();
            glTranslatef(x + jx, 0.0f, z + jz);
            glScalef(scale, scale, scale);
            drawTree(0.0f, 0.0f, treeType);
            glPopMatrix();
        }
    }
}
...
void drawStaircase(float xOuter, float xInner, float zFront, float zBack)
{
    ...
    //  ΓöÇΓöÇ STEP GEOMETRY ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    // Flight 1: ground ΓåÆ mid landing
    for (int i = 0; i < N; i++) {
        float f = (float)i / N;
        float yb = f * yMid;
    ...
    // Flight 2: mid landing ΓåÆ top balcony
    for (int i = 0; i < N; i++) {
        float f = (float)i / N;
    ...
    //  ΓöÇΓöÇ RAILING POSTS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    // Flight 1 inner edge (midX_Inner side) ΓÇö previously missing
    for (int i = 0; i <= N * 3; i++) {
        float f = (float)i / (N * 3);
        float yL = f * yMid;
        float zL = zFront - f * (zFront - zBack);
    ...
    // Flight 2 inner edge (midX_Outer side) ΓÇö previously missing
    for (int i = 0; i <= N * 3; i++) {
        float f = (float)i / (N * 3);
        float yU = yMid + f * (yTop - yMid);
        float zU = zBack + f * (zFront - zBack);
    ...
    // Mid-landing: horizontal top rail caps ΓÇö front and outer side only
    // Front cap
    ...
    // Outer side (x = xOuter, posts along Z) ΓÇö outward direction
    ...
    // NOTE: No railing at z=tZ1 ΓÇö that is the staircase entrance opening!

    //  ΓöÇΓöÇ SLOPED HANDRAILS (top rails on each flight) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    glColor3f(0.85f, 0.85f, 0.85f);
    glLineWidth(4.5f);
    glBegin(GL_LINES);
    // Flight 1 ΓÇö outer rail
    glVertex3f(xInner, 0.0f + rH, zFront);
    glVertex3f(xInner, yMid + rH, zBack);
    // Flight 1 ΓÇö inner rail
    glVertex3f(midX_Inner, 0.0f + rH, zFront);
    glVertex3f(midX_Inner, yMid + rH, zBack);
    // Flight 2 ΓÇö outer rail
    glVertex3f(xOuter, yMid + rH, zBack);
    glVertex3f(xOuter, yTop + rH, zFront);
    // Flight 2 ΓÇö inner rail
    glVertex3f(midX_Outer, yMid + rH, zBack);
    glVertex3f(midX_Outer, yTop + rH, zFront);
    glEnd();

    //  ΓöÇΓöÇ DRAIN PIPES ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    glColor3f(1.0f, 1.0f, 1.0f);
    glLineWidth(3.5f);
    glBegin(GL_LINES);
...
void drawBuilding()
{
    ...
    // ΓöÇΓöÇ CONCRETE ACCESS RAMP (right side of building front) ΓöÇΓöÇ
