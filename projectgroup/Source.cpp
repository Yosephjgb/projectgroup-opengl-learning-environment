...
void drawWelcomePage() {
    ...
    renderBitmapString(175, 480, GLUT_BITMAP_HELVETICA_18, "3D Simulation of CCI-Building – OpenGL / GLUT");
    ...
    renderBitmapString(310, 438, GLUT_BITMAP_HELVETICA_18, "CONTROLS");

    // --- 6. Key bindings table ---
    ...
    renderBitmapString(descX, rowY, GLUT_BITMAP_HELVETICA_12,
        "Start cinematic morning scene (sunrise → lecture)");
    rowY -= rowH;
    ...
    glColor3f(0.85f, 0.85f, 0.90f);
    renderBitmapString(descX, rowY, GLUT_BITMAP_HELVETICA_12,
        "Go inside classroom (student perspective)");
    rowY -= rowH;
    ...
    renderBitmapString(descX, rowY, GLUT_BITMAP_HELVETICA_12,
        "Rotate camera / zoom / walk inside class");
    rowY -= rowH;
    ...
    // --- 8. Group Members Header & Names ---
    glColor3f(0.95f, 0.85f, 0.35f);
    renderBitmapString(325, rowY - 10, GLUT_BITMAP_HELVETICA_18, "GROUP MEMBERS");

    glColor3f(0.75f, 0.80f, 0.85f);
    // Two columns for names
    const char* membersLeft[3] = { "Yoseph Girma    1009/16", "Tamirat Kassa   0739/16", "Shanbal Zinabe   0708/16" };
    const char* membersRight[3] = { "Ruth Melkamu    0668/16", "Tinsae Mesfin   0896/16" , "Sekina Mohammed   0693/16"};

    float nameY = rowY - 30;
    for (int i = 0; i < 3; i++) {
        renderBitmapString(220, nameY, GLUT_BITMAP_HELVETICA_12, membersLeft[i]);
        nameY -= 20.0f;
    }

    nameY = rowY - 30;
    for (int i = 0; i < 3; i++) {
        renderBitmapString(440, nameY, GLUT_BITMAP_HELVETICA_12, membersRight[i]);
        nameY -= 20.0f;
    }

    // Group Badge
    glColor3f(0.60f, 0.65f, 0.70f);
    renderBitmapString(345, rowY - 85, GLUT_BITMAP_HELVETICA_18, "Group 3");

    // --- 9. Footer credits ---
    glColor3f(0.40f, 0.42f, 0.48f);
    renderBitmapString(230, 85, GLUT_BITMAP_HELVETICA_12,
        "Computer Graphics & Computer Project — 2026");
